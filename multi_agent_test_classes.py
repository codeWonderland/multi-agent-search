"""PA 3 specific autograding test classes.
...
from ghost_agents import DirectionalGhost
from ghost_agents import RandomGhost  # noqa
import layout
import pacman
...
class MultiagentTreeState:
    ...
    def __init__(self, problem, state):
        """Initialize tree state from problem and state."""
        self.problem = problem
        self.state = state

    def generate_successor(self, agent_index, action):
        """Generate successor state by having an agent take an action."""
        if VERBOSE:
            print("generate_successor(%s, %s, %s) -> %s" %
                  (self.state, agent_index, action,
                   self.problem.state_to_successor_map[self.state][action]))
    ...
    def get_score(self):
        """Return score or raise exception if non-terminal or not max depth."""
        if VERBOSE:
            print("get_score(%s) -> %s" %
                  (self.state, self.problem.evaluation[self.state]))

        if self.state not in self.problem.evaluation:
            raise Exception('get_score() called on non-terminal state'
                            ' or before maximum depth achieved.')
        return float(self.problem.evaluation[self.state])

    def get_legal_actions(self, agent_index=0):
        """Return legal actions of agent from current state."""
        if VERBOSE:
            print("get_legal_actions(%s) -> %s" %
                  (self.state, self.problem.state_to_actions[self.state]))
        return list(self.problem.state_to_actions[self.state])

    def is_win(self):
        """Return if state is a winning state."""
        if VERBOSE:
            print("is_win(%s) -> %s" %
                  (self.state, self.state in self.problem.win_states))
        return self.state in self.problem.win_states

    def is_lose(self):
        """Return if state is a losing state."""
        if VERBOSE:
            print("is_lose(%s) -> %s" %
                  (self.state, self.state in self.problem.lose_states))
        return self.state in self.problem.lose_states

    def get_num_agents(self):
        """Return number of agents in the problem."""
        if VERBOSE:
            print("get_num_agents(%s) -> %s" %
                  (self.state, self.problem.num_agents))
        return self.problem.num_agents


class MultiagentTreeProblem(object):
    """Problem class for multiagent tree search."""

    def __init__(self, num_agents, start_state, win_states, lose_states,
                 successors, evaluation):
        """Initialize problem given all relevant information."""
    ...
    def reset(self):
        """Reset problem so only the start state has been generated."""
        self.generated_states = set([self.start_state.state])


def parse_tree_problem(test_dict):
    """Create MultiagentTreeProblem from test_dict information."""
    ...
    return MultiagentTreeProblem(num_agents, start_state, win_states,
                                 lose_states, successors, evaluation)


def run(lay, lay_name, pac, ghosts, disp, n_games=1, name='games'):
    """Run a few games and return/output their statistics."""
    starttime = time.time()
    print('*** Running %s on' % name, lay_name, '%d time(s).' % n_games)
    games = pacman.run_games(lay, pac, ghosts, disp, n_games, False,
                             catch_exceptions=True, timeout=120)
    print('*** Finished running %s on' % name, lay_name,
...
class GradingAgent(Agent):
    """Class for grading a student's agent code."""

    def __init__(self, seed, student_agent, optimal_actions, alt_depth_actions,
                 partial_ply_bug_actions):
        """Create agent from student agent and reference values."""
        ...
        # boolean vectors represent types of implementation the student
        # could have
        self.actions_consistent_with_optimal =\
            [True for _ in range(len(optimal_actions[0]))]
        self.actions_consistent_with_alternative_depth =\
            [True for _ in range(len(alt_depth_actions[0]))]
        self.actions_consistent_with_partial_ply_bug =\
            [True for _ in range(len(partial_ply_bug_actions[0]))]
    ...
    def register_initial_state(self, state):
        """Register initial state of student's agents and seed random."""
    ...
    def get_action(self, state):
        ...
        if (not current_right_states_explored and
                self.wrong_states_explored < 0):
            self.wrong_states_explored = 1
        ...
        if not student_optimal_action:
            self.suboptimal_moves.append((state, student_action[0],
    ...
    def check_failure(self):
        """Check if the student's agent failed.

        Return +n if have n suboptimal moves.
               -1 if have only off by one depth moves.
               0 otherwise.
        """
...
class PolyAgent(Agent):
    """Agent class for preparing solutions."""

    def __init__(self, seed, multi_agents, our_pac_options, depth):
        """Initialize Agent given options."""
        # prepare our pacman agents
        solution_agents, alternative_depth_agents, partial_ply_bug_agents =\
            self.construct_our_pacs(multi_agents, our_pac_options)
    ...
    def construct_our_pacs(self, multi_agents, keyword_dict):
        """Construct pacman agents (called by __init__)."""
        pacs_without_stop =\
        ...
        alternative_depth_pacs = self.select(pacs_with_stop +
                                             pacs_without_stop,
                                             [1, 4, 2, 5])
        return (ourpac, alternative_depth_pacs, partial_ply_bug_pacs)

    def register_initial_state(self, state):
        """Register initial state of our agents and seed random."""
    ...
    def get_action(self, state):
        """Get optimal action and record data."""
        # survey agents
        GameState.get_and_reset_explored()
        optimal_action_lists = []
        for agent in self.solution_agents:
            optimal_action_lists.append(
                (agent.get_best_pacman_actions(state)[0],
                 len(GameState.get_and_reset_explored())))
        alternative_depth_lists = [agent.get_best_pacman_actions(state)[0]
                                   for agent in self.alternative_depth_agents]
        partial_ply_bug_lists = [agent.get_best_pacman_actions(state)[0]
                                 for agent in self.partial_ply_bug_agents]
        # record responses
    ...
    def get_traces(self):
        ...
        return (self.optimal_action_lists, self.alternative_depth_lists,
                self.partial_ply_bug_lists)


class PacmanGameTreeTest(test_classes.TestCase):
    """Class for testing multiagent search on Pacman games."""

    def __init__(self, question, test_dict):
        """Initialize test case.

        Extends test_classes.TestCase.__init__
        """
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        all_actions = [json.loads(x) for x in
                       solution_dict['optimal_actions'].split('\n')]
        alt_depth_actions = [json.loads(x) for x in
                             solution_dict['alt_depth_actions'].split('\n')]
        partial_ply_bug_actions = [json.loads(x) for x in
                                   solution_dict['partial_ply_bug_actions'
                                                 ].split('\n')]
        ...
        stats = run(lay, self.layout_name, pac, [DirectionalGhost(i + 1)
                                                 for i in range(2)],
                    disp, name=self.alg)
        if stats['timeouts'] > 0:
        ...
        if code == 0:
            ...
            if pac.wrong_states_explored >= 0:
    ...
    def write_list(self, handle, name, list):
        """Write list line by line to file handle."""
        handle.write('%s: """\n' % name)
        for l in list:
            handle.write('%s\n' % json.dumps(l))
        handle.write('"""\n')
    ...
    def write_solution(self, module_dict, file_path):
        ...
        Overrides Overrides test_classes.TestCase.write_solution
        """
        ...
        disp = self.question.display
        run(lay, self.layout_name, pac, [DirectionalGhost(i + 1)
                                         for i in range(2)],
            disp, name=self.alg)
        optimal_actions, alt_depth_actions, partial_ply_bug_actions =\
            pac.get_traces()
        ...
        self.write_list(handle, 'partial_ply_bug_actions',
                        partial_ply_bug_actions)
        handle.close()


class GraphGameTreeTest(test_classes.TestCase):
    """Class for testing multiagent search on graphs."""
    ...
    def solve_problem(self, multi_agents):
        """Run student code to get initial action."""
        self.problem.reset()
        student_agent = getattr(multi_agents, self.alg)(depth=self.depth)
    ...
    def add_diagram(self):
        """Add depiction of the graph to the test."""
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if generated != gold_generated:
            self.add_message('Incorrect generated nodes for depth=%s' %
                             (self.depth,))
...
class EvalAgentTest(test_classes.TestCase):
    """Class for testing student's evaluation function."""
    ...
    def __init__(self, question, test_dict):
        ...
        self.score_minimum = (int(test_dict['score_minimum'])
                              if 'score_minimum' in test_dict else None)
        self.non_timeout_minimum = (int(test_dict['non_timeout_minimum'])
                                    if 'non_timeout_minimum' in test_dict
                                    else None)
        self.wins_minimum = (int(test_dict['wins_minimum'])
                             if 'wins_minimum' in test_dict else None)

        self.score_thresholds = [int(s) for s in
                                 test_dict.get('score_thresholds', '').split()]
        self.non_timeout_thresholds = [int(s) for s in
                                       test_dict.get('non_timeout_thresholds',
                                                     '').split()]
        self.wins_thresholds = [int(s) for s in
                                test_dict.get('wins_thresholds', '').split()]

        self.points_per_threshold = int(test_dict.get('points_per_threshold',
                                                      1))

        self.max_points = sum([len(t) * self.points_per_threshold for t in
                               [self.score_thresholds,
                                self.non_timeout_thresholds,
                                self.wins_thresholds]])
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        results = [grade_threshold(average_score, self.score_minimum,
                                   self.score_thresholds, "average score"),
                   grade_threshold(non_timeouts, self.non_timeout_minimum,
                                   self.non_timeout_thresholds,
                                   "games not timed out"),
                   grade_threshold(wins, self.wins_minimum,
                                   self.wins_thresholds, "wins")]
...
def extra_lint(grades):
    """Perform extra, assignment specific, linting."""
    docstring = sys.modules["multi_agents"].better_evaluation_function.__doc__

    for line in docstring.split("\n"):
        if "description" in line.lower():
            if ((line.split(":")[1].strip() ==
                 "<write something here explaining your approach>" or
                 line.split(":")[1].strip() == "")):
                grades.fail("FAIL: Missing 'Description:' in "
                            "multi_agents.better_evaluation_function's "
                            "docstring")
