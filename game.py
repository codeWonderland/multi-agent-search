...
class Agent:
    ...
    def __init__(self, index=0):
        """Create agent labeled with given index."""
        self.index = index

    def get_action(self, state):
        """Return action from given state.
...
class Directions:
    """Class for storing valid actions and helpers."""
...
class Configuration:
    """A Configuration holds a character's configuration.

    This is the (x,y) coordinate of the character, along with its traveling
    direction.

    The convention for positions, like a graph, is that (0,0) is the lower left
    corner, x increases horizontally and y increases vertically.
    Therefore, NORTH is the direction of increasing y, or (0,1).
    """

    def __init__(self, position, direction):
        """Create a Configuration from given position and direction."""
    ...
    def is_integer(self):
        """Return True iff position x and y coordinates are both integers."""
        x, y = self.position
        return x == int(x) and y == int(y)

    def __eq__(self, other):
        """Override == to return True iff position and directions are equal."""
        if other is None:
            return False
    ...
    def __hash__(self):
        """Override hash function to use position and direction."""
        return hash(str(self))

    def __str__(self):
        """Return string of this Configuration."""
        return ("(x,y)=" + str(self.position) + ", " +
                str(self.direction))

    def generate_successor(self, vector):
        """Generate a new configuration by applying the action vector.

        Note:  This is a low-level call and does not attempt to respect the
               legalityof the movement.

        vector: Action movement vector.
        """
        x, y = self.position
        dx, dy = vector
...
class AgentState:
    ...
    def __init__(self, start_configuration, is_pacman):
        """Create an AgentState in given start configuration."""
    ...
    def __str__(self):
        """Return string of AgentState."""
        if self.is_pacman:
            return "Pacman: " + str(self.configuration)
        else:
            return ("Ghost: " + str(self.configuration) + "; " +
                    str(self.scared_timer))

    def __eq__(self, other):
        """Override == operator to consider configuration and scared timer."""
        if other is None:
            return False
        return str(self) == str(other)

    def __hash__(self):
        """Override hash to consider configuration and scared timer."""
        return hash(str(self))
    ...
    def get_position(self):
        """Get position or return None if not defined."""
        if self.configuration is None:
            return None
        return self.configuration.position

    def get_direction(self):
        """Get direction or return None if not defined."""
        if self.configuration is None:
            return None
...
class Grid:
    """A 2-D array of objects backed by a list of lists.
    ...
    def __init__(self, width, height, initial_value=False,
        ...
        If bit_representation is given then values filled in from that
        """
        ...
        self.width = width
        self.height = height
        self.data = [[initial_value for _ in range(height)]
    ...
    def __str__(self):
        """Return string representation of grid oriented like Pacman board."""
    ...
    def __eq__(self, other):
        """Override == to return True if data are equal."""
        if other is None:
            return False
        return self.data == other.data

    def __hash__(self):
        """Compute hash of grid."""
        # return hash(str(self))
        base = 1
        h = 0
        for l in self.data:
    ...
    def deep_copy(self):
        """Make a deep_copy of this grid (same as copy)."""
        return self.copy()

    def shallow_copy(self):
        """Make a shallow copy of this grid: stores reference to same data."""
        g = Grid(self.width, self.height)
        g.data = self.data
        return g
    ...
    def as_list(self, key=True):
        """Return list containing (x, y) tuples of key locations."""
        list = []
    ...
    def pack_bits(self):
        """Return an efficient int list representation.
    ...
    def _unpack_bits(self, bits):
        """Fill in data from a bit-level representation."""
        cell = 0
        for packed in bits:
...
def reconstitute_grid(bit_representation):
    """Reconstitute Grid from given bit_representation."""
    if not isinstance(bit_representation, tuple):
        return bit_representation
    width, height = bit_representation[:2]
    return Grid(width, height, bit_representation=bit_representation[2:])
...
class Actions:
    ...
    # We need to set the order to match what tests were generated at.
    # This is because order matters when running alpha-beta pruning.
    # It's a bit of a hack, but otherwise would need to re-code all the
    # variants of common mistakes that the test offers
    @staticmethod
    def _sort(direction_tuple):
        return [Directions.WEST, Directions.STOP, Directions.EAST,
                Directions.NORTH, Directions.SOUTH].index(direction_tuple[0])

    _directions_as_list = sorted(_directions.items(), key=_sort.__func__)

    TOLERANCE = .001

    @staticmethod
    def reverse_direction(action):
        """Return reverse direction of given action."""
    ...
    def vector_to_direction(vector):
        """Return direction from given movement vector."""
        dx, dy = vector
        if dy > 0:
            return Directions.NORTH
        if dy < 0:
    ...
    def direction_to_vector(direction, speed=1.0):
        """Return movement vector from given direction."""
    ...
    def get_legal_neighbors(position, walls):
        """Return list of possible neighbors from given position."""
        x, y = position
        x_int, y_int = int(x + 0.5), int(y + 0.5)
        neighbors = []
        for dir, vec in Actions._directions_as_list:
    ...
    def get_successor(position, action):
        """Get successor position of taking action."""
...
class GameStateData:
    ...
    def __init__(self, prev_state=None):
        """Generate a new data packet by copying information from previous."""
    ...
    def copy_agent_states(self, agent_states):
        """Return list copies from list of agent states."""
        copied_states = []
    ...
    def __eq__(self, other):
        """Override == to compare two states."""
        if other is None:
            return False
    ...
    def __hash__(self):
        ...
        for i, state in enumerate(self.agent_states):
            try:
                ...
                print(e)
                # hash(state)
        return int((hash(tuple(self.agent_states)) + 13 * hash(self.food) +
    ...
    def __str__(self):
        ...
        for agent_state in self.agent_states:
            ...
            if agent_state.configuration is None:
                continue
            x, y = [int(i) for i in
                    util.nearest_point(agent_state.configuration.position)]
    ...
    def initialize(self, layout, num_ghost_agents):
        """Create an initial game state from a layout array.
        ...
        for is_pacman, pos in layout.agent_positions:
            if not is_pacman:
                if num_ghosts == num_ghost_agents:
                    ...
                    num_ghosts += 1
            self.agent_states.append(AgentState(Configuration(pos,
                                                              Directions.STOP),
                                                is_pacman))
...
class Game:
    ...
    def _agent_crash(self, agent_index, quiet=False):
        """Hhandle agent crashes (helper method)."""
        if not quiet:
            traceback.print_exc()
        self.game_over = True
    ...
    def mute(self, agent_index):
        """Set stdout/stderr to be agent output."""
    ...
    def run(self):
        ...
        while not self.game_over:
            ...
            if self.catch_exceptions:
                try:
                    self.state = self.state.generate_successor(agent_index,
                                                               action)
                except Exception as data:
                    self.mute(agent_index)
                    self._agent_crash(agent_index)
